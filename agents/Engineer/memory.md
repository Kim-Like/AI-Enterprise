...
- Consolidation complete: external business repos migrated into `programs/`
- Control-plane DB: `father.db` (WAL)
- Runtime: FastAPI + Uvicorn on `0.0.0.0:8001`
- Objective intake: `POST /api/execute`
...
3. `error_log` contains historical open errors from earlier sessions and should be triaged.
...
- Do not move app transactional data into `father.db`.
